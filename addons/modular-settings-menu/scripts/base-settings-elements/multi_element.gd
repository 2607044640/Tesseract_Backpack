...
func _enter_tree() -> void:
	ParentRef = owner
	ParentRef.setting_changed.connect(update_element)
	ParentRef.apply_button_pressed.connect(apply_settings)
	ParentRef.SettingsMenuRef.changes_discarded.connect(load_settings)
	SettingsDataManager.settings_retrieved.connect(load_settings)
	init_main_element()
	init_sub_elements()
