...
func _ready():
	...
	SettingsMenuRef.settings_menu_opened.connect(get_settings)
	SettingsMenuRef.apply_button_pressed.connect(on_apply_settings)
	SettingsMenuRef.settings_menu_closed.connect(clear_cache)
...
func settings_changed(elementId: String) -> void:
	...
	setting_changed.emit(elementId)
